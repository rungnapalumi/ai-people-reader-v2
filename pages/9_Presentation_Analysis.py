## pages/9_Presentation_Analysis.py
# Presentation Analysis page (copy of People Reader, new scoring)
...
def presentation_analysis_fixed_settings() -> Dict[str, Any]:
...
def apply_finished_job_output_keys(group_id: str, outputs: Dict[str, str]) -> None:
    ...
    m = st.session_state.get("presentation_analysis_jobs_by_group") or {}
    entry: Dict[str, Any] = {}
...
def ensure_session_defaults() -> None:
    defaults = {
        "presentation_analysis_last_group_id": "",
        "presentation_analysis_submission_id_override": "",
        "presentation_analysis_last_name": "",
        "presentation_analysis_audience_mode": "one",
        "presentation_analysis_jobs_by_group": {},
        "presentation_analysis_notify_email": (os.getenv("PEOPLE_READER_DEFAULT_NOTIFY_EMAIL") or "").strip(),
    }
...
def clear_state() -> None:
    for k in (
        ...
        "presentation_analysis_submission_id_override",
        "presentation_analysis_last_name",
        "presentation_analysis_download_id",
        "presentation_analysis_audience_mode",
        "presentation_analysis_video_upload",
        "presentation_analysis_jobs_by_group",
        "presentation_analysis_processed_upload_id",
    ):
...
name_value = st.text_input(
    "Name",
    value=str(st.session_state.get("presentation_analysis_last_name") or ""),
    placeholder="Enter your name",
)

if name_value:
    st.session_state["presentation_analysis_last_name"] = name_value

report_notify_email = st.text_input(
    "Email (Please check in your junkmail if it's not in the inbox)",
    key="presentation_analysis_notify_email",
...
audience_mode = st.radio(
    ...
    horizontal=True,
    key="presentation_analysis_audience_mode",
)

# Presentation Analysis does not use the 10-type movement-profile rubric.
# The scorer reads pose features for each clip and maps them directly to H/M/L.
movement_type_mode = ""
...
uploaded = st.file_uploader(
    ...
    key="presentation_analysis_video_upload",
)
...
_already_processed_upload = str(
    st.session_state.get("presentation_analysis_processed_upload_id") or ""
)
...
if run:
    ...
    with st.status("Uploading video to cloud...", expanded=True) as status:
        ...
        st.session_state["presentation_analysis_processed_upload_id"] = _upload_signature
        st.session_state["presentation_analysis_last_group_id"] = group_id
        st.session_state["presentation_analysis_submission_id_override"] = group_id
        if "presentation_analysis_jobs_by_group" not in st.session_state:
            st.session_state["presentation_analysis_jobs_by_group"] = {}
        st.session_state["presentation_analysis_jobs_by_group"][group_id] = {
            "dots_job_id": job_dots_id,
            "skeleton_job_id": job_skeleton_id,
            "report_job_id": job_report_id,
        }
        if "presentation_analysis_langs_by_group" not in st.session_state:
            st.session_state["presentation_analysis_langs_by_group"] = {}
        st.session_state["presentation_analysis_langs_by_group"][group_id] = list(langs)
...
current_group_id = _sanitize_group_id(
    st.session_state.get("presentation_analysis_submission_id_override")
    or st.session_state.get("presentation_analysis_last_group_id")
    or ""
)
...
if has_prev and st.button("Clear and Start New Upload", key="presentation_analysis_clear", type="secondary"):
    clear_state()
    st.rerun()
...
if current_group_id:
    ...
    _langs_sel = (st.session_state.get("presentation_analysis_langs_by_group") or {}).get(current_group_id)
    ...
    if overall_pct >= 100:
        ...
        st_autorefresh(
            interval=15_000,
            key=f"presentation_analysis_autorefresh_{current_group_id}",
        )
    ...
    if expect_dots:
        render_download_button(
            ...
            filename="dots.mp4",
            mime="video/mp4",
            button_key="presentation_analysis_dl_dots",
            ready=dots_ready,
        )
    if expect_skeleton:
        render_download_button(
            ...
            mime="video/mp4",
            button_key="presentation_analysis_dl_skeleton",
    ...
    if wants_th:
        render_download_button(
            ...
            filename="Presentation_Analysis_report_TH.pdf",
            mime="application/pdf",
            button_key="presentation_analysis_dl_report_th_pdf",
    ...
    if wants_en:
        render_download_button(
            ...
            filename="Presentation_Analysis_report_EN.pdf",
            mime="application/pdf",
            button_key="presentation_analysis_dl_report_en_pdf",
