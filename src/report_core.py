    ...
    def append_graph_pages_for_operation_test() -> None:
        ...
        for idx, (graph_title, graph_path) in enumerate(graph_specs):
            ...
            if graph_path and os.path.exists(graph_path):
                ...
                write_line("Graph image unavailable", size=11, gap=16)
            # Move credit to the last operation-test page (page 4) at the same bottom-right position.
    ...
    if report.first_impression:
        ...
        if is_operation_test:
            if is_thai:
                ...
                write_bullet(f"{eye_label} (Eye Contact)", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
                    f"ระดับ: {_first_impression_level_th(fi.eye_contact_pct, metric='eye_contact')}",
                    LEVEL_STYLE,
                    extra_gap=0,
                )
                write_bullet(f"{upright_label} (Uprightness)", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
                    f"ระดับ: {_first_impression_level_th(fi.upright_pct, metric='uprightness')}",
                    LEVEL_STYLE,
                    extra_gap=0,
                )
                write_bullet(f"{stance_label} (Stance)", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
                ...
                write_bullet("Eye Contact", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
                    f"Scale: {_first_impression_level(fi.eye_contact_pct, metric='eye_contact')}",
                    LEVEL_STYLE,
                    extra_gap=0,
                )
                write_bullet("Uprightness", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
                    f"Scale: {_first_impression_level(fi.upright_pct, metric='uprightness')}",
                    LEVEL_STYLE,
                    extra_gap=0,
                )
                write_bullet("Stance", indent=28, space_after=0, bullet_text="•")
                write_paragraph_block(
