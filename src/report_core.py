...
def build_docx_report(report: ReportData, output_bio: io.BytesIO, graph1_path: str, graph2_path: str, lang: str = "en"):
    ...
    texts = {
        "title": "รายงานการวิเคราะห์บุคลิกภาพ" if is_thai else "Character Analysis Report",
        "client_name": "ชื่อลูกค้า:" if is_thai else "Client Name:",
        "analysis_date": "วันที่วิเคราะห์:" if is_thai else "Analysis Date:",
        "video_info": "ข้อมูลวิดีโอ" if is_thai else "Video Information",
        "duration": "ระยะเวลา:" if is_thai else "Duration:",
        "detailed_analysis": "การวิเคราะห์โดยละเอียด" if is_thai else "Detailed Analysis",
        "first_impression": "1.  ความประทับใจแรกพบ" if is_thai else "1.  First impression",
        ...
        "indicators": "ตัวชี้วัดเชิงบวกจากทั้งหมด" if is_thai else "positive indicators out of",
        "total_indicators": "ตัวชี้วัดทั้งหมด" if is_thai else "total indicators",
        "effort_title": "ผลลัพธ์การตรวจจับการเคลื่อนไหวแบบ Effort" if is_thai else "Effort Motion Detection Results",
        "shape_title": "ผลลัพธ์การตรวจจับการเคลื่อนไหวแบบ Shape" if is_thai else "Shape Motion Detection Results",
    ...
    if report.first_impression:
        ...
        # For Thai, add simplified bullets (full Thai translation would be very long)
        if is_thai:
            doc.add_paragraph("• การสบตาของคุณมีความมั่นคง อบอุ่น และมุ่งเน้นที่ผู้ฟังตลอดการนำเสนอ")
            doc.add_paragraph("• คุณรักษาการสบตาโดยตรงในจุดสำคัญของข้อความ ซึ่งช่วยเพิ่มความไว้วางใจและความชัดเจน")
            doc.add_paragraph("• การเปลี่ยนสายตาของคุณเป็นไปอย่างมีจุดประสงค์และเป็นธรรมชาติ")
            doc.add_paragraph("• ไม่มีสัญญาณของการหลีกเลี่ยง การสบตาสนับสนุนความมั่นใจและความน่าเชื่อถือ")
        else:
    ...
    if report.first_impression:
        ...
        # For Thai, add simplified bullets
        if is_thai:
            doc.add_paragraph("• คุณรักษาท่าทางที่ตั้งตรงอย่างเป็นธรรมชาติตลอดการนำเสนอ")
            doc.add_paragraph("• หน้าอกเปิดกว้าง ไหล่ผ่อนคลาย และศีรษะอยู่ในแนวที่ดี — ส่งสัญญาณถึงความสมดุล ความพร้อม และอำนาจ")
            doc.add_paragraph("• แม้ในขณะทำท่าทาง การจัดแนวในแนวตั้งของคุณยังคงมั่นคง แสดงถึงการควบคุมแกนกลางที่ดี")
            doc.add_paragraph("• ไม่มีการก้มหรือยุบตัว ซึ่งสนับสนุนลักษณะที่เป็นมืออาชีพ")
        else:
    ...
    if report.first_impression:
        ...
        # For Thai, add simplified bullets (only first 2)
        if is_thai:
            doc.add_paragraph("• ท่ายืนของคุณสมมาตรและมั่นคง เท้าวางห่างประมาณความกว้างไหล่")
            doc.add_paragraph("• การเปลี่ยนน้ำหนักถูกควบคุมและน้อยมาก ป้องกันการรบกวนและแสดงความมั่นใจ")
        else:
